...
case "$KEY" in
  "$PREFIX"*) ;;
  # أول ثلاثة محارف وطول ما لُصق يكفيان لمعرفة الخطأ (أمر منسوخ بدل المفتاح، أو لصق ناقص) دون كشف مفتاح
  *) die "هذا لا يشبه مفتاح $LABEL (يبدأ عادة بـ $PREFIX). ما لُصق يبدأ بـ «${KEY:0:3}» وطوله ${#KEY} محرفاً — انسخ المفتاح من صفحة $LABEL أولاً ثم أعد الأمر. لم يتغيّر شيء" ;;
esac

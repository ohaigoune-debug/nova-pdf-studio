...
export async function generateMetadata() {
  ...
  return { title: tt('auth.resetTitle') }
}
...
export default async function ResetPasswordPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  if (!token) error = errorMessage('RESET_TOKEN_INVALID', locale)
  else {
    try {
      await verifyPasswordResetToken(await getDb(), token)
    } catch (e) {
      error = isAppError(e) ? errorMessage(e.code, locale) : errorMessage('INTERNAL', locale)
    }
  }
  return (
    <AuthCard

        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {overview.map((g) => (
            <Card key={g.groupId}>
              <CardHeader>
                <CardTitle>
                  <Link href={`/assistant/students?groupId=${g.groupId}`} className="hover:underline">
                    {g.name}
                  </Link>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                <div className="grid grid-cols-4 gap-2 text-center text-xs">
                  ...
                  <div className="rounded-md bg-destructive/10 p-2">
                    ...
                    <p className="text-muted-foreground">{t('assistant.unexcusedShort')}</p>
                  </div>
                ...
                {recent.map((s) => (
                  <TableRow key={s.id}>
                    <TableCell className="tabular">
                      <Link href={`/assistant/sessions/${s.id}`} className="hover:underline">

...
create policy users_self on users for select using (id = app_current_user() or app_current_role() in ('SUPER_ADMIN','TEACHER'));
drop policy if exists profiles_self on profiles;
create policy profiles_self on profiles for all using (user_id = app_current_user() or app_current_role() in ('SUPER_ADMIN','TEACHER'));
...
create policy timeline_scope on student_timeline for select using (
  ...
  or (app_current_role() = 'TEACHER' and workspace_id = app_current_workspace())
